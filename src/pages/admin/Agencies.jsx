...
import { WHOLESALE_PRICE } from '../../lib/resellerSaas'
...
export default function Agencies() {
  ...
  const totals = useMemo(
    () => ({
      ...
      ourMrr: agencies.reduce((s, a) => s + (a.ourRevenue || 0), 0),
      gross: agencies.reduce((s, a) => s + (a.gross || 0), 0),
      margin: agencies.reduce((s, a) => s + (a.saasMargin || 0), 0),
    }),
    [agencies],
  )
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">
            {agencies.length} total · {money(totals.ourMrr)} MRR to CaseLift
          </p>
        </div>
      ...
      {loading ? (
        ...
        <Table
          ...
          rows={rows.map((a) => [
            ...
            money(a.gross || 0),
            money(a.ourRevenue || 0),
            <span className={(a.saasMargin || 0) >= 0 ? 'text-emerald-300' : 'text-rose-300'}>
              {money(a.saasMargin || 0)}
            </span>,
            ...
            <div className="flex items-center gap-1.5" onClick={stop}>
              ...
              <button
                ...
                className="rounded-md border border-surface-700 bg-surface-800 px-2 py-1 text-xs text-sky-300 transition hover:bg-surface-700 disabled:opacity-40"
                title="Bill now"
              >
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div>
          <label className="label">Wholesale rate per active subaccount (USD)</label>
...
function EditRateModal({ agency, onClose, onSaved }) {
  ...
  async function save() {
    ...
    const { error: err } = await supabase
      ...
      .update({ reseller_wholesale_price: value, monthly_fee: value })
      .eq('id', agency.id)
    if (err) {
  ...
  return (
    <Modal
      title={`Wholesale rate — ${agency.name}`}
      onClose={onClose}
      maxWidth="max-w-md"
      footer={
        <>
    ...
    >
      <div className="space-y-3">
        <p className="text-sm text-slate-400">
          What CaseLift bills this reseller per active subaccount, per month. Default is {money(WHOLESALE_PRICE)}.
        </p>
        <div>
